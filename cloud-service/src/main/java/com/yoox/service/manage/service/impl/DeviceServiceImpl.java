...
import com.yoox.api.firmware.AbstractFirmwareService;
import com.yoox.api.property.AbstractPropertyService;
...
import com.yoox.great.mqtt.handle.events.EventsSubscribe;
...
import com.yoox.great.mqtt.handle.osd.OsdSubscribe;
import com.yoox.great.mqtt.handle.property.PropertySetReplyResultEnum;
import com.yoox.great.mqtt.handle.property.PropertySetSubscribe;
import com.yoox.great.mqtt.handle.requests.RequestsSubscribe;
import com.yoox.great.mqtt.core.SDKManager;
import com.yoox.great.mqtt.handle.services.ServicesReplyData;
import com.yoox.great.mqtt.handle.services.ServicesSubscribe;
import com.yoox.great.mqtt.handle.services.TopicServicesResponse;
import com.yoox.great.mqtt.handle.state.StateSubscribe;
import com.yoox.great.mqtt.handle.status.StatusSubscribe;
...
import com.yoox.service.manage.dao.IDeviceMapper;
import com.yoox.service.manage.model.dto.*;
import com.yoox.service.manage.model.entity.DeviceEntity;
import com.yoox.service.manage.model.enums.DeviceFirmwareStatusEnum;
import com.yoox.service.manage.model.enums.PropertySetFieldEnum;
import com.yoox.service.manage.model.param.DeviceQueryParam;
import com.yoox.service.manage.model.receiver.BasicDeviceProperty;
...
public class DeviceServiceImpl implements IDeviceService {
    ...
    public List<DeviceDTO> getDevicesTopoForWeb(String workspaceId) {
        ...
        // Always splice persisted topology, including offline remote
        // controllers. Online state and device identity are separate concerns.
        devicesList.forEach(this::spliceDeviceTopo);
    ...
    public void spliceDeviceTopo(DeviceDTO gateway) {
        ...
        if (DeviceDomainEnum.REMOTER_CONTROL == gateway.getDomain()) {
            gateway.setRemoteControllerSn(gateway.getDeviceSn());
        }
        ...
        List<DeviceDTO> subDevices = getDevicesByParams(
                DeviceQueryParam.builder().deviceSn(gateway.getChildDeviceSn()).build());
        if (subDevices.isEmpty()) {
            log.warn("Gateway {} references missing child device {}.",
    ...
    private void addFirmwareStatus(DeviceDTO deviceDTO, DeviceEntity entity) {
        ...
        if (progressOpt.isPresent()) {
            ...
            deviceDTO.setFirmwareProgress(progressOpt.map(EventsReceiver::getOutput)
                    ...
                    .map(com.yoox.great.mqtt.model.firmware.OtaProgressData::getPercent)
                    .orElse(0));
            return;
        }
    ...
    public Boolean bindDevice(DeviceDTO device) {
        ...
        // 支持预登记：设备不存在则创建，存在则更新
        int count = mapper.selectCount(
                new LambdaQueryWrapper<DeviceEntity>().eq(DeviceEntity::getDeviceSn, device.getDeviceSn()));
        boolean ok = count > 0 ? this.updateDevice(device) : this.saveDevice(device) > 0;
        if (!ok) {
            return false;
        }

        // 设备未在线时跳过拓扑推送，但仍视为预登记成功

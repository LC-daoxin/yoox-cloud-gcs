...
| 7 | **固件未实现该 method** | 同链路同时段对照指令（如 camera_screen_drag）秒回，而目标 method 的**所有报文变体**（字段增减、双 topic、need_reply 等）均零回复 | 2026-08-12 camera_look_at 案例：9 种变体实测全部被丢弃，结论为 RC 1.9.1.203 未实现。前端曾降级为 camera_screen_drag 闭环，当前已按操作要求恢复原生 camera_look_at；如再次出现 211001，使用 A/B 脚本复测。判定前必须先发对照指令排除链路/权限因素 |
